...
```{r}
library(ggplot2)
ggplot(mapping = aes(x = 1:15081, y = degree_distribution(g)))+
  geom_point()+
  scale_y_log10()+
  scale_x_log10()
...
Aha, this already tells us a more interesting story. As we can see, indeed most degrees are between 1 and 1,000.
The shape we observed in the plot above is very typical for real-world networks and known as a [*power-law distribution*](https://en.wikipedia.org/wiki/Power_law).
